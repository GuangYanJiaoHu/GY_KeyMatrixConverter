 ...
 * beginning of execution however, if there is a chance of HIDAPI handles being opened by different ths simultaneously.
 *
 ...
 * \return returns an quint32 id number, or 0 if unsuccessful.
 */
quint32 QHidApiPrivate::open(int ser_number, ushort vendorId, ushort productId, QString serialNumber)
{
    //qDebug()<<"--------------------------------251-----------------------------";
    QList<QMultiMap<ushort, QVariant> > products = mVendorMap.values(vendorId);

    return openNewProduct(ser_number,vendorId, productId, serialNumber);
...
void QHidApiPrivate::close(quint32 id) {
    ...
    if (dev != NULL) {
        hid_close(dev);
        clear_all();
        //qDebug()<<"关闭端口"<<dev<<mIdDeviceMap.count();
    }
    else
    {
        //qDebug()<<"端口NULL";
...
quint32 QHidApiPrivate::openNewProduct(int ser_number, ushort vendorId, ushort productId, QString serialNumber)
{
//qDebug()<<"--------------------------------625-----------------------------";
    hid_device *device = NULL;
    quint32 id = 0;
    if (serialNumber.isEmpty()) {
        ...
        //qDebug()<<"获取device";
        device = hid_open_ser_number(ser_number,vendorId, productId, NULL);
    ...
    while (vmapIt.hasNext()) {
        ...
        for(auto i = pmap.begin(); i != pmap.end(); i++){
            ...
            if (p == productId) {
                ...
                QVariant v = i.value();
                QMetaType t =  v.metaType();
                bool ok;
                if (t.UInt == QMetaType::UInt) {
                    id = v.toUInt(&ok);
                    if (ok) {
                        if (mIdDeviceMap.contains(id))
                            return id;
                    }
                } else if (t.Char == QMetaType::Type()) {
