#include "GY_HIDMouse.h"

//读取设备信息，当设备连接成功后必须要做的一步
void GY_HIDMouse::GetDeviceReadMessage(const QString &portPath)
{
    for(auto item = listConnectDevice->begin(); item != listConnectDevice->end(); item++){
        if(item->getDevicePortPath() == portPath){
            ...
            QTimer::singleShot(50, this,[=](){  this->WriteHandle(item->getDeviceHandle(), QByteArray("FE F5 06")); }); //设备唯一编码
...
QList<HidDeviceInfo> *GY_HIDMouse::getListConnectDevice()
{
    return listConnectDevice;
}

#pragma region=================================== 工 具 方 式 类 ============================================{
bool GY_HIDMouse::Write(const QString &portPath, const QString &data, enum HidDeviceInfo::DeviceType deviceType)
{
    for(auto item = listConnectDevice->begin(); item != listConnectDevice->end(); item++){
        if(item->getDevicePortPath() == portPath){
...
bool GY_HIDMouse::Write(const QString &portPath, const QByteArray &data, enum HidDeviceInfo::DeviceType deviceType)
{
    for(auto item = listConnectDevice->begin(); item != listConnectDevice->end(); item++){
        if(item->getDevicePortPath() == portPath){
...
bool GY_HIDMouse::Write(const QString &portPath, const QStringList &data, enum HidDeviceInfo::DeviceType deviceType)
{
    for(auto item = listConnectDevice->begin(); item != listConnectDevice->end(); item++){
        if(item->getDevicePortPath() == portPath){
...
void GY_HIDMouse::slotsMouseConnect(const QString &portPath)
{
    ...
    QTimer::singleShot(200,this,[=](){
        for(int i = 0; i < this->getListConnectDevice()->count(); i++){
            if(this->getListConnectDevice()->at(i).getDevicePortPath() == portPath){
                emit signalMouseConnection(this->getListConnectDevice()->at(i));
                break;
            }
        }
    });
...
void GY_HIDMouse::setDeviceMessage(const QString &portPath, const QString &Data, const QString &order, enum HidDeviceInfo::DeviceType deviceType)
{
    ...
    for(auto item = listConnectDevice->begin(); item != listConnectDevice->end(); item++){
        if(item->getDevicePortPath() == portPath){
            if(order == "FE F5 02"){
                ...
                qDebug() << "HID鼠标返回信息-设备颜色：" << QString(HexToMessage(recvData.remove("FE F5 02", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
                ...
                qDebug() << "HID鼠标返回信息-出厂时间：" << QString(HexToMessage(recvData.remove("FE F5 03", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
                ...
                qDebug() << "HID鼠标返回信息-生产厂家：" << QString(HexToMessage(recvData.remove("FE F5 04", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
                ...
                qDebug() << "HID鼠标返回信息-设备批号：" << QString(HexToMessage(recvData.remove("FE F5 05", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
                ...
                qDebug() << "HID鼠标返回信息-唯一编码：" << QString(HexToMessage(recvData.remove("FE F5 06", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
                ...
                qDebug() << "HID鼠标返回信息-检测步骤：" << QString(HexToMessage(recvData.remove("FE F5 07", Qt::CaseInsensitive))).simplified().remove(QChar('\u0000'));
            }else if(order == "FE FF"){
                ...
                qDebug() << "HID鼠标返回信息-DFU版本：" << item->getDeviceName() << " - " << item->getDeviceDfuVersion();
            }else if(order == "FE F6"){
                ...
                if(batterType == "1"){
                    ...
                    if(batterLevel != item->getDeviceBatteryLevel() || isBatterType != item->getDeviceChargingState()){
                        ...
                        emit signalBatteryChange(item->getDevicePortPath(), item->getDeviceBatteryLevel(), true, HidDeviceInfo::DeviceType::_HID_MOUSE);
                    }
                    ...
                    if(batterLevel != item->getDeviceBatteryLevel() || isBatterType != item->getDeviceChargingState()){
                        item->setDeviceBatteryLevel(batterLevel);
                        emit signalBatteryChange(item->getDevicePortPath(), item->getDeviceBatteryLevel(), false, HidDeviceInfo::DeviceType::_HID_MOUSE);
                    }
                    item->setDeviceChargingState(false);
                }
                qDebug() << "HID鼠标返回信息-电量信息：" << batterLevel << " -充电状态- " << isBatterType;
            }
            break;
        }
    }
...
//----------------------------------HID鼠标读取----------------------------
void GY_HIDMouse::slotReadyRead(const QString &portPath, QString &recvData)  //hid_device *Handle,
{
    qDebug() << "--HID鼠标读取数据--：" << recvData;
    if(recvData.left(8) == "FE F5 02" || recvData.left(8) == "fe f5 02"){
        this->setDeviceMessage(portPath, recvData, "FE F5 02", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(8) == "FE F5 03" || recvData.left(8) == "fe f5 03"){
        this->setDeviceMessage(portPath, recvData, "FE F5 03", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(8) == "FE F5 04" || recvData.left(8) == "fe f5 04"){
        this->setDeviceMessage(portPath, recvData, "FE F5 04", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(8) == "FE F5 05" || recvData.left(8) == "fe f5 05"){
        this->setDeviceMessage(portPath, recvData, "FE F5 05", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(8) == "FE F5 06" || recvData.left(8) == "fe f5 06"){
        this->setDeviceMessage(portPath, recvData, "FE F5 06", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(8) == "FE F5 07" || recvData.left(8) == "fe f5 07"){
        this->setDeviceMessage(portPath, recvData, "FE F5 07", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(14) == "4C 69 67 68 74" || recvData.left(14) == "4c 69 67 68 74"){       //这里由于硬件传输问题需要将FE FF 改为获取FE 4c
        this->setDeviceMessage(portPath, recvData, "FE FF", HidDeviceInfo::_HID_MOUSE);
    }else if(recvData.left(5) == "FE F6" || recvData.left(5) == "fe f6"){
        this->setDeviceMessage(portPath, recvData, "FE F6", HidDeviceInfo::_HID_MOUSE);
    }
}
