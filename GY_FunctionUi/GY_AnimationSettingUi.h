...
private slots:
    ...
    void on_pushButton_StaticExportWeb_clicked();                       //静态动画网页版导出
    ...
    void on_checkBox_StaticSend_stateChanged(int arg1);                 //静态动画键盘模拟开启
    void on_checkBox_DynamicSend_stateChanged(int arg1);                //动态动画键盘模拟开启


signals:
    void signalAnimationStaticSimulate(QString path, QStringList listStaticPictureName, bool isStart, bool isDownLoadKeyboard);      //静态动画模拟
    void signalAnimationDynamicSimulate(QString path, QStringList listDynamicPicturePath, bool isStart, bool isDownLoadKeyboard);    //动态动画模拟
    void signalAnimationStaticOnlySimulate(QString path, QString onlyName, bool isDownLoadKeyboard);                                 //静态动画逐帧模拟
    void signalAnimationDynamicOnlySimulate(QString path, QString onlyName, bool isDownLoadKeyboard);                                //动态动画逐帧模拟
    ...
    void signalAnimationStaticWebExport(QStringList path);                                                     //静态动画网页版导出
...
private:
    ...
    bool isDownloadKeyboardStatic, isDownloadKeyboardDynamic;   //动画下发键盘模拟
};
